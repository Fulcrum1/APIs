...
func createSingleWord(word, translation, pronunciation, language string, userId uint) (int64, string, int) {
	if word == "" || translation == "" {
	...
	wordID, err := AddWordToDatabase(word, translation, pronunciation, languageId, userId)
	if err != nil {
...
func CreateWord(c *gin.Context) {
	...
	wordID, errMsg, status := createSingleWord(req.Word, req.Translation, req.Pronunciation, req.Language, req.UserID)
	if status != 201 {
...
func CreateWords(c *gin.Context) {
	...
	for _, word := range req.Words {
		wordID, errMsg, status := createSingleWord(word.Word, word.Translation, word.Pronunciation, req.Language, req.UserID)
		if status != 201 {
...
func AddWordToDatabase(word string, translation string, pronunciation string, language uint, userId uint) (uint, error) {
