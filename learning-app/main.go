...
func main() {
	...
	router.DELETE("/words", func(c *gin.Context) {
		initializers.DB.Exec("DELETE FROM list_words")
		initializers.DB.Exec("DELETE FROM words")
		c.JSON(http.StatusOK, gin.H{"message": "Words deleted"})
	})
	// Words routes
	...
	protected.PUT("/card/param-card/:id", controller.UpdateCardParam)
